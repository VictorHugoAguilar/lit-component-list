...
import { uuid } from "./mixin/utils/index.js";
import { FormControlMixin } from "./mixin/form/index.js";
import { ChoiceGroupMixin } from "./mixin/form-choice-group/index.js";
import { SlotMixin, ScopedElementsMixin } from "./mixin/core/index.js";
import { FormRegistrarMixin } from "./mixin/form-registration/index.js";

import { VhaaOptions } from "./VhaaOptions";
...
/**
 * @type {ListboxMixin}
 * @param {import('@open-wc/dedupe-mixin').Constructor<import('@Vhaa/core').LitElement>} superclass
 */
const ListboxMixinImplementation = (superclass) =>
    /**
     * `vhaa-listbox` Description
     *
     * @customElement
     * @polymer
     * @demo
     *
     */
    class VhaaListbox extends FormControlMixin(
        ScopedElementsMixin(ChoiceGroupMixin(SlotMixin(FormRegistrarMixin(LitElement))))
    ) {
        ...
        get slots() {
            return {
                ...super.slots,
                input: () => {
                    const vhaaOptions = /** @type {import('./VhaaOptions.js').VhaaOptions} */ (
                        this.createScopedElement("vhaa-options")
                    );
                    vhaaOptions.setAttribute("data-tag-name", "vhaa-options");
                    vhaaOptions.registrationTarget = this;
                    return vhaaOptions;
                },
            };
        }
         ...
         */
        get _listboxNode() {
            // Cast to unknown first, since HTMLElementWithValue is not compatible with VhaaOptions
            return /** @type {VhaaOptions} */ (/** @type {unknown} */ (this._inputNode));
        }
         ...
         * @param {FormControlHost & VhaaOption} child
        ...
        addFormElement(child, indexToInsertAt) {
            ...
            this.__proxyChildModelValueChanged(
                /** @type {CustomEvent & { target: FormControlHost & vhaaOption; }} */ ({
                    target: child,
                })
            );
        ...
        _handleTypeAhead(ev, { setAsChecked }) {
            ...
            if (code.startsWith("Key") || code.startsWith("Digit") || code.startsWith("Numpad")) {
                ...
                const matchedItemIndex =
                    ...
                    this.formElements.findIndex((el) =>
                        el.modelValue.value.toLowerCase().startsWith(chars)
                    );
        ...
        /**
         * @overridable
         * @param {Event & { target: VhaaOption }} ev
         * @protected
         */
        ...
        VhaaOption detects clicks itself and could now potentially be checked as offline dom.
        ...
        again and disable click handling inside VhaaOption (since there would be no use case for
        'orphan' options)
         */
        ...
        /**
         * @param {VhaaOption|VhaaOption[]} [exclude]
         * @protected
         */
        ...
        /**
         * @param {Event & { target: VhaaOption }} cfgOrEvent
         * @private
         */
        ...
        /**
         * @param {CustomEvent & { target: VhaaOption; }} ev
         * @private
         */
                ...
                this.dispatchEvent(
                    new CustomEvent("model-value-changed", {
                        detail: /** @type {ModelValueEventDetails} */ ({
                            ...
                            isTriggeredByUser:
                                ev.detail.isTriggeredByUser || this._isHandlingUserInput,
                            element: ev.target,
                        }),
                    })
                );
